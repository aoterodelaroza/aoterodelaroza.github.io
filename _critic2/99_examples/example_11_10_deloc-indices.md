...
DI calculation in critic2). Because it does not make sense to have
`ecutrho` higher than 4 times `ecutwfc` in a NC calculation, and the
cube files have to have grids with the same dimensions, use `ecutrho`
equal to 4 times `ecutwfc` in both the PAW and the NC calculations.
...
former undergoes an additional symmetrization step inside QE.
