...
* If symmetry is used in `pw.x`, then the density obtained from the
`rho.cube` generated by `pp.x` and that calculated as the sum of the
squares of the MLWFs are slightly different. This is because the
former undergoes an additional symmetrization step inside QE. In
practice, the results are essentially the same regardless of whether
`nosym` is used or not. Note that using symmetry may bump the number
of points in the real-space grid to have more efficient FFT, so either
you use `nosym=.true.` in both the PAW and the NC SCF calculations or
you do not use it in either.
