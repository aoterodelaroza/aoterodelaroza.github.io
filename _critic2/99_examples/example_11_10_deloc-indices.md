...
used or not, so if you have a large system you may be better off
without the `nosym=.true.`. Note that using symmetry may bump the
number of points in the real-space grid to have more efficient FFT, so
either you use `nosym=.true.` in both the PAW and the NC SCF
calculations or you do not use it in either.
