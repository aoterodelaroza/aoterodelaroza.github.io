...
### VASP (POSCAR, CONTCAR, vasp, CHGCAR, CHG, AECCAR*, ELFCAR, PARCHG)

In VASP, the crystal structure can be read from a number of files,
including the `POSCAR` (recommended), the `CONTCAR`, a file with
`.vasp` extension, the `CHGCAR`, and
...
`AECCAR2`, `PARCHG`, and `ELFCAR` are also accepted
(e.g. `nacl.POSCAR` or `POSCAR_nacl`).
...
### Quantum ESPRESSO (scf.in, scf.out, pwi, pwo)

In addition to reading Gaussian cube files generated by Quantum
ESPRESSO, the outputs (extension `.scf.out` or `.pwo`) and inputs
(`.scf.in` or `.pwi`) of
...
For Quantum ESPRESSO inputs given as `file.scf.in`, only the `.in` extension
is detected.
...
### SIESTA Inputs (fdf) {#c2-fdf}

Critic2 reads the structure in SIESTA input files (extension
`.fdf`), following the same rules as SIESTA. The lattice is built
from `LatticeConstant` and either the `LatticeParameters` or the
`LatticeVectors` block. If neither block is given, `LatticeConstant`
alone defines a cubic cell, and a file with no lattice information at
all is read as a molecule. The atoms are read from the
`AtomicCoordinatesAndAtomicSpecies` block, in any of the units allowed
by `AtomicCoordinatesFormat`, or from a `Zmatrix` block (molecule,
cartesian, scaled, and fractional sections, with symbolic constants,
variables, and constraints, and the `ZM.UnitsLength` and
`ZM.UnitsAngle` units). The species come from the
`ChemicalSpeciesLabel` block, and their labels become the species
names in critic2. The `AtomicCoordinatesOrigin` shift (an explicit
vector or `COP`, `COM`, and `MIN`, optionally restricted to some
directions) and the `SuperCell` block are applied. Unlike in SIESTA,
non-diagonal supercell matrices are accepted. Ghost species (negative
atomic number) are read as atoms with atomic number zero, and
synthetic species (atomic number above 200) take the element from the
beginning of their label.

The fdf syntax is handled as in the libfdf library used by SIESTA:
labels are case-insensitive and the `.`, `_`, and `-` characters in
them are ignored, the first occurrence of a label is the one used, the
`#`, `!`, and `;` characters start a comment, and the `%include
file`, `%block label < file`, and `label < file` directives are
followed. File names in these directives are looked up relative to
the directory of the file that contains them, then relative to the
working directory, and finally by their base name in the directory
of the file that contains them. Values with units (such as
`LatticeConstant`) must give the unit explicitly, as in SIESTA.
...
Crystal geometries can be read from CASTEP input (`.cell`) files,
CASTEP phonon (`.phonon`) files as well as from the outputs of a
geometry optimization (`.geom`), from where the last structure is
read. The system is always assumed to be a crystal.

### Xyz files (xyz) {#c2-xyz}

Crystal geometries in xyz format (`.xyz`) generated by
[ASE](https://wiki.fysik.dtu.dk/ase/) can be read by critic2. For a
xyz structure to be detected as a crystal, a `Lattice` and a `pbc`
entries must be present in the second line. The lattice parameters are
read from `Lattice` and the rest of the information is ignored.

### Magres files (magres) {#c2-magres}

Critic2 can read crystal geometries in magres format (`.magres`)
compatible with those read by
[magresview](https://www.ccpnc.ac.uk/magresview/magresview/magres_view.html).
At present, only the `atoms` tag is read.

### ALAMODE input files (alm.in) {#c2-alamode}

Critic2 can read crystal geometries from an [ALAMODE](https://alamode.readthedocs.io/en/latest/)
input file. The `&general`, `&cell`, and `&positions` namelists are read.

### AkaiKKR input files (in) {#c2-akaikkr}

Critic2 can read crystal geometries from an
[AkaiKKR](http://kkr.issp.u-tokyo.ac.jp/) input file. AkaiKKR input
files seem to have no extension so please rename them to have the
`.in` extension for critic2 to detect the format or read them with the
`AKAIKKR` keyword to force this particular format, if no extension is
used:
```
CRYSTAL AKAIKKR file
```

### SPRKKR/xband system files (sys) {#c2-xband}

The system files (with extension `.sys`) generated by the xband
program (used along with the
...
- `VASP`: a VASP-style (POSCAR, CHGCAR, vasp,...) structure file.
