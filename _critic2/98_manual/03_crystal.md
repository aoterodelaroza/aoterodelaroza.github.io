...
website. The extension `.16` correspons to a `fort.16` file generated
by [DMACRYS](http://www.chem.ucl.ac.uk/cposs/dmacrys/index.html),
which also uses the SHELX format.

### DMACRYS/NEIGHCRYS files (21)

The `fort.21` file generated by NEIGHCRYS, part of the
[DMACRYS](http://www.chem.ucl.ac.uk/cposs/dmacrys/index.html) package
can be read by critic2. The file is created during a distributed
multipole calculation in molecular crystals, and it is useful to
...
### DMACRYS input file (dmain)

The crystal structure can be read from a DMACRYS main input file, with
extension `.dmain`. [DMACRYS](http://www.chem.ucl.ac.uk/cposs/dmacrys/index.html)
is a program for modeling molecular crystals using rigid molecules and
