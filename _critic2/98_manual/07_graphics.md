...
option to GRID is used for shifting the origin of the grid to a
different point. The origin of the shifted grid is the position of
point `ix.i`, `iy.i`, `iz.i` in the old grid.

The `NEWCELL` option to `CUBE GRID` is used to create supercells or swap
crystallographic axes when writing the cell. The `a.s`, `b.s` and
`c.s` must be one of "a", "b" or "c" and `na.i`, `nb.i` and `nc.i` are
...
zero. `NEWCELL` can only be used with `CUBE GRID`.
